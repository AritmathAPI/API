...
class SyntacticAnalyzer:
    ...
    def _factor(self) -> ASTNode:
        ...
        if not token: raise SyntaxError("Unexpected end of expression")
    ...
    def evaluate(self, node: Optional[ASTNode] = None) -> float:
        if node is None: node = self.parse()

        return self._evaluate_node(node)

    def _evaluate_node(self, node: ASTNode) -> float:
        if node.node_type == "NUMBER":   return node.value
        if node.node_type == "GROUP":    return self._evaluate_node(node.left)
    ...
    def _collapse_and_record_steps(self, current_node: ASTNode, root_for_string: ASTNode):
    ...
    def _node_to_string(self, node: ASTNode) -> str:
        ...
        if node.node_type == "BINARY_OP":
            ...
            if node.left.node_type == "BINARY_OP" and self.PRECEDENCE[node.left.value] < self.PRECEDENCE[node.value]:
                left_str = f"({left_str})"
            if node.right.node_type == "BINARY_OP" and self.PRECEDENCE[node.right.value] < self.PRECEDENCE[node.value]:
